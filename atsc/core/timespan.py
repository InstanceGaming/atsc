...
from utils import compact_dt, text_to_enum, conditional_text
...
def is_midnight(dt: datetime) -> bool:
    return dt.hour == 0 and dt.minute == 0 and dt.second == 0 and \
           dt.microsecond == 0
...
class Timespan:
    ...
    @property
    def tz_aware(self):
        return self.start.tzinfo is not None and self.start.utcoffset() is \
               not None and self.end.tzinfo is not None and \
               self.end.utcoffset() is not None
    ...
    def __repr__(self):
        return f'<timespan {self.getDurationText()}' \
               f'{conditional_text(self.getWeekdaysText())}' \
               f'{conditional_text(self.getMonthsText())}' \
    ...
    def getDurationText(self) -> str:
        return f'{compact_dt(self.start)} to ' \
               f'{compact_dt(self.end)}'
...
def parse_timespan_text(raw_calendar_value,
                        ...
                        raw_start_text: str,
                        raw_end_text: str,
                        tz: tzinfo) -> Tuple[Optional[Timespan],
                                             Optional[str]]:
    weekdays = None
    months = None
                ...
                if shorthand_text is not None:
                    ...
                    if weekdays_csv_text is not None:
                        ...
                        for csvalue in weekdays_csv:
                            weekday_enum = text_to_enum(Weekday, csvalue)
                ...
                if shorthand_text is not None:
                    ...
                    if months_csv_text is not None:
                        ...
                        for csvalue in months_csv:
                            month_enum = text_to_enum(Month, csvalue)
